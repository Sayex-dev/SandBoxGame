...
public partial class BiomeWorldGenerator : WorldGenerator
{
	...
	private void PopulateChunk(Chunk chunk, Vector3I chunkLocation, Vector3I chunkSize)
	{
		...
		for (int x = 0; x < chunkSize.X; x++)
		{
			for (int z = 0; z < chunkSize.Z; z++)
			{
				...
				int maxY = Mathf.Min(groundHeight - chunkLocation.Y * chunkSize.Y, chunkSize.Y);
